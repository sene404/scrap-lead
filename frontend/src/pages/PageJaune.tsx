...
const PageJaune: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    saveAs(blob, "resultats_pagejaune.xlsx");
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch("http://localhost:4000/api/pj", {
        method: "POST",
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-8">
      <div className="text-center">
        <h1 className="text-4xl font-bold text-gray-900">🟨 PageJaune</h1>
        <p className="text-lg text-gray-700 mt-2">
          Interface de scraping pour les Page Jaune.
        </p>
      </div>
      ...
      {loading && (
        <div className="text-center text-rose-600 font-semibold animate-pulse">
          ⏳ Recherche en cours sur Pages Jaunes...
        </div>
      )}
